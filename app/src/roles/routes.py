...
router_cache: Cache = Cache(owner=router, all_prefix="roles", ttl=settings.default_cache_ttl)
...
async def read_roles(name: str = Query(default=None),
    ...
    roles: List[RoleResponse] = await router_cache.get(key=cache_key)
    if not roles:
        roles = await roles_repository.read_roles(
                                                            name=name,
                                                            domain=domain,
                                                            db=db)
        await router_cache.set(key=cache_key, value=roles)
    if not roles:
