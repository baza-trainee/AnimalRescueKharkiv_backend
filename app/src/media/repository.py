...
class MediaRepository:
    ...
    async def save_blob(self, blob_data: BinaryIO, db: AsyncSession) -> uuid.UUID:    
        ...
        for chunk_index in range(0, len(bytes_array), chunk_size):
    ...
    async def create_media_asset(self, file: UploadFile, db: AsyncSession) -> MediaAsset:
        """
        Creates and saves to database a media asset enity with extension, content type and reference to binary blob from an uploaded file. Returns the created media asset
        """
    ...
    async def read_media_assets(self, from_date: datetime, to_date: datetime, media_type: str, extension: str, skip: int, limit: int, db: AsyncSession) -> list[MediaAsset]:
        """
        Reads a media asset enity by its id from database. Returns the retrieved media asset
        """
